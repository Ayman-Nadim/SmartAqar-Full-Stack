...
//       // Form will be reset by useEffect when property prop changes
//     } catch (error) {
...
//     if (newImages.length === 0 && !property) {
//       setImageError('At least one image is required');
//     } else {
//       setImageError('');
//     }
...
//                 <span className="text-xs text-gray-500 ml-1">(Max 3 images)</span>
//               </label>
...
const PropertyForm = ({ 
  ...
  useEffect(() => {
    if (property) {
      ...
      // Remplir les URLs existantes
      propertyImages.forEach((url, index) => {
        if (index < 3) imageUrls[index] = url;
      });
  ...
  const validateForm = () => {
    ...
    // Image validation for new properties
    const validImages = selectedImages.filter(url => url.trim() !== '');
    if (!property && validImages.length === 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    const propertyData = {
      ...
      bathrooms: parseInt(formData.bathrooms),
      area: parseInt(formData.area),
      images: selectedImages.filter(url => url.trim() !== '') // Filtrer les URLs vides
    };
  ...
  const handleImageUrlChange = (index, url) => {
    ...
    const validImages = newImages.filter(u => u.trim() !== '');
    if (!property && validImages.length === 0) {
      setImageError('At least one image URL is required');
    } else {
      setImageError('');
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Property Images {!property && '*'}
                <span className="text-xs text-gray-500 ml-1">(Max 3 image URLs)</span>
              </label>
                ...
                {selectedImages.map((imageUrl, index) => (
                  <div key={index}>
                    <div className="flex items-center space-x-2">
                      <span className="text-sm text-gray-600 w-16">Image {index + 1}:</span>
                      <input
                        type="url"
                        value={imageUrl}
                        ...
                        className={`flex-1 px-3 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent ${
                          imageUrl && !isValidImageUrl(imageUrl) ? 'border-red-500' : 'border-gray-300'
                        }`}
                        disabled={isLoading}
                      />
                      {imageUrl && (
                    ...
                    {/* Validation message pour URL invalide */}
                    {imageUrl && !isValidImageUrl(imageUrl) && (
                      <p className="text-red-600 text-xs mt-1 ml-18">
              ...
              {/* Aperçu des images */}
              {selectedImages.some(url => url.trim() !== '') && (
                <div className="mt-4">
                  <p className="text-sm font-medium text-gray-700 mb-3">
                    Image Preview ({selectedImages.filter(url => url.trim() !== '').length}/3)
                  </p>
                  <div className="grid grid-cols-3 gap-3">
                    {selectedImages.map((imageUrl, index) => (
                      imageUrl.trim() !== '' && (
              ...
              <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                <p className="text-xs text-blue-700 mb-2">
                  <strong>Exemples d'URLs d'images valides :</strong>
                </p>
                <div className="space-y-1 text-xs text-blue-600">
                  <p>• https://images.unsplash.com/photo-1570129477492-45c003edd2be</p>
                  <p>• https://example.com/villa.jpg</p>
                  <p>• https://mysite.com/images/apartment.png</p>
                </div>
              </div>
            </div>
